...
# determine accepted object indices
accepted_object_indices = {} # maps category_mapping_index to 0-based incrementing array index
f = open("accepted_object_mappings.txt", "r")
counter = 0
for line in f:
  tokens = line.split(" ")
  index = int(tokens[-1])
  accepted_object_indices[index] = counter
  counter += 1
f.close()

num_object_categories = 289
...
# print out final cooc matrix results
...
axes[0].matshow(cooccurrence_matrix)
...
for i in range(len(normalised_matrix)):
  for j in range(len(normalised_matrix)):
    ...
    if i == j:
      normalised_matrix[i][j] = 0

# # thresholding
# for i in range(len(normalised_matrix)):
#   for j in range(len(normalised_matrix)):
#     if normalised_matrix[i][j] < 0.6:
#       normalised_matrix[i][j] = 0

axes[1].matshow(normalised_matrix)
...
axes[1].title.set_text("Normalised")

# plt.show()

# EXPLORATION:
print("pairs with the highest occurrence:")
for i in range(len(normalised_matrix)):
  for j in range(i + 1, len(normalised_matrix)):
    if normalised_matrix[i][j] > 0.95:
      print(object_words[i], " || ", object_words[j])